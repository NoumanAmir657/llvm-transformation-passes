...
void registerAnalyses(FunctionAnalysisManager &FAM) {
    ...
    FAM.registerPass([] { return addmetadata::AddMetadataAnalysis(); });
    FAM.registerPass([] { return addtosub::AddToSubAnalysis(); });
    FAM.registerPass([] { return multoshift::MulToShiftAnalysis(); });
    FAM.registerPass([] { return addcall::AddCallAnalysis(); });
}
...
bool registerPipeline(StringRef Name, FunctionPassManager &FPM,
    ...
    if (Name == "print<add-metadata>") {
        FPM.addPass(addmetadata::AddMetadataPrinterPass(errs()));
        return true;
    }

    if (Name == "add-metadata") {
        FPM.addPass(addmetadata::AddMetadataPass());
        return true;
    }

    if (Name == "print<add-to-sub>") {
        FPM.addPass(addtosub::AddToSubPrinterPass(errs()));
        return true;
    }

    if (Name == "add-to-sub") {
        FPM.addPass(addtosub::AddToSubPass());
        return true;
    }

    if (Name == "print<mul-to-shift>") {
        FPM.addPass(multoshift::MulToShiftPrinterPass(errs()));
        return true;
    }

    if (Name == "mul-to-shift") {
        FPM.addPass(multoshift::MulToShiftPass());
        return true;
    }

    if (Name == "print<add-call>") {
        FPM.addPass(addcall::AddCallPrinterPass(errs()));
        return true;
    }

    if (Name == "add-call") {
        FPM.addPass(addcall::AddCallPass());
        return true;
    }
...
PassPluginLibraryInfo getAddConstPluginInfo() {
    return {LLVM_PLUGIN_API_VERSION, "Passes", LLVM_VERSION_STRING,
            [](PassBuilder &PB) {
